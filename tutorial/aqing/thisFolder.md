...
輕輕鬆鬆在 10 分鐘內就能獲得**本地運行的**大語言模型。

## fromZero-to-coarseRAG.ipynb
這是一整套從單句嵌入向量到分割一篇文章 (.txt) 並進行最高相似度搜索的操作指南，即所以在 `/exp/aqing/` 下的東西的大成 。  
Especially `simimlarityDemo.py` and `queryDemo.py`.  
過程中省略了學習路上會寫出的很醜程式，整體來說稍微美麗一點。  
相依的文件路經需要與這個腳本放在同一個資料夾下。  
總之是一個直接落地實作的好東西。