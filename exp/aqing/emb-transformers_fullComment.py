...
'''
inp = tokenizer(testString_3, return_tensors="pt") # 'pt' for pytorch
...
out = ebModel(**inp) # **代表全部帶入, inp 裡面有
...
'''
sentence_emb = meanPooling(out, inp['attention_mask']) # Perform pooling, 把 N 個 token_emb 壓成一條 sentence_emb
sentence_emb_3 = torch.nn.functional.normalize(sentence_emb, p=2, dim=1) # Normalize embeddings

print(f"Sentence embeddings: {sentence_emb_3}")
